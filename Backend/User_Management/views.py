...
class UserView(APIView):
    ...
    def post(self, request):
        try:
            ...
            InfoSerialized = InfoSerializer(
                UserObj.info, data=request.data, partial=True
            )
            ...
            if UserSerialized.is_valid():
                ...
                if (
                    FormData.get("profile_img") is not None
                    and ancien_img != DEFAULT_PROFILE_IMG
                ):
    ...
    def get_user(request):
        try:
            ...
            if request.user.pk == friend.pk:
                ...
                try:
                    userObj["friendship"] = request.user.get_friendship(
                        friend=friend
                    ).status
                except:
